...
circ.mean  <- round(mean(combined_files$Circ.),3)
circ.sd    <- round(sd(combined_files$Circ.),3)

round.mean <- round(mean(combined_files$Round),3)
round.sd   <- round(sd(combined_files$Round),3)
...
export           <- data.frame(d, sigma, n, circ.mean, circ.sd, round.mean, round.sd)
colnames(export) <- c("d", "\u03C3", "n", "circ", "circ sd", "round", "round sd")
rownames(export) <- c("")
...
my_theme <- function (base_size = 22, base_line_size = 1) {
    theme_bw(base_size = base_size,  base_family = "") %+replace%
        theme(
            axis.text.x = element_text(
                size    = base_size,
                margin  = margin(t = 0.75 * base_size, b = 0.25 * base_size),
                color   = "black"
            ),
            axis.text.y = element_text(
                size    = base_size,
                margin  = margin(r = 0.75 * base_size, l = 0.25 * base_size),
                color   = "black"
            ),
...
#need to rewrite to be consistent in plot area

xmid <- df.hst$d[which(df.hst$density == max(df.hst$density))]
xmin <- floor(xmid - 0.25 * xmid)
xmax <- ceiling(xmid + 0.35 * xmid)
xann <- xmid + 0.07 * xmid
